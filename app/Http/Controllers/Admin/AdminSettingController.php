...
class AdminSettingController extends Controller
{
    ...
    public function logo_submit(Request $request)
    {
        $setting = Setting::where('id',1)->first();
        $request->validate([
            'logo' => ['required','image','mimes:jpg,jpeg,png,gif','max:10240'], // 10MB max
        ]);
    ...
    public function favicon_submit(Request $request)
    {
        $setting = Setting::where('id',1)->first();
        $request->validate([
            'favicon' => ['required','image','mimes:jpg,jpeg,png,gif,ico','max:10240'], // 10MB max
        ]);
    ...
    public function banner_submit(Request $request)
    {
        $setting = Setting::where('id',1)->first();
        $request->validate([
            'banner' => ['required','image','mimes:jpg,jpeg,png,gif','max:10240'], // 10MB max
        ]);
        $final_name = 'banner_'.time().'.'.$request->banner->extension();
        $request->banner->move(public_path('uploads'), $final_name);

        // Delete old banner if it exists and is not the default
        if($setting->banner && file_exists(public_path('uploads/'.$setting->banner))) {
    ...
    {
        $request->validate([
            'ticket_purchase_expire_date' => ['required','date'],
        ]);
        $setting = Setting::where('id',1)->first();
